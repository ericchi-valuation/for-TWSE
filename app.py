...
def safe_get(df, idx, keys, default=0):
    """V6.7 無敵資料萃取器：自動找尋同義欄位，杜絕 KeyError 崩潰"""
    if idx not in df.index: return default
    for k in keys:
        if k in df.columns and pd.notna(df.loc[idx, k]): return df.loc[idx, k]
    return default
...
def get_historical_metrics(stock, hist_data):
    try:
        if hist_data.empty: return ["-"]*4, 0, 0
        ...
        if fin.empty or bs.empty: return ["-"]*4, 0, 0
        ...
        for r_date in fin.index:
            try:
                if r_date not in hist_data.index:
                    ...
                    p = hist_data.iloc[nearest]['Close']
                else: p = hist_data.loc[r_date]['Close']
        ...
        return [fmt_rng(c_pe), fmt_rng(c_pb), fmt_rng([v for v in ps_vals if 0<v<150]), fmt_rng(evebitda_vals)], np.mean(c_pe) if c_pe else 0, min(c_pb) if c_pb else 0
    except: return ["-"]*4, 0, 0
...
def get_3_stage_valuation(stock, is_fin, real_g):
    try:
        ...
        wacc = max((eq/(eq+debt))*max(0.035+(stock.info.get('beta',1.0)*0.06),0.07) + (debt/(eq+debt))*0.025, 0.08 if is_fin else 0.0)
        ...
        return max((dcf - (debt if not is_fin else 0) + (cash if not is_fin else 0)) / (stock.info.get('sharesOutstanding',1) or 1), 0), g1, wacc, roic
    except: return 0, 0, 0.1, 0

def calculate_scores(info, real_g, qoq_g, upside, cur_pe, cur_ev, avg_pe, med_pe, cur_pb, min_pb, wacc, roic, debt_ebitda, op_m):
    ...
    if 0 < info.get('pegRatio', 0) < 1.5: s['G'] += 2

    s['Total'] = (s['Q']*wq*10) + (s['V']*wv*10) + (s['G']*wg*10)
    return s

def compile_stock_data(symbol, ind, stock, info, price, real_g, qoq_g, wacc, roic, ranges, avg_pe, min_pb, cur_pe, cur_ev, intrinsic, upside, eps, is_fin, med_pe):
    ...
    scores = calculate_scores(info, real_g, qoq_g, upside, cur_pe, cur_ev, avg_pe, med_pe, cur_pb, min_pb, wacc, roic, debt/ttm_ebitda if ttm_ebitda > 0 else 0, op_margins)
    ...
    return {
        ...
        '營業利益率': f"{info.get('operatingMargins', 0)*100:.1f}%", 
...
def run_pit_backtest(sym, stock, target_date, is_finance):
    try:
        ...
        ep, cp = hist[hist.index >= target_dt]['Close'].iloc[0], hist['Close'].iloc[-1]
        ...
        ke = max(0.035 + stock.info.get('beta', 1.0)*0.06, 0.07)
        ic = equity + debt - cash
        ...
        scores = calculate_scores(stock.info, real_growth, qoq_growth, upside, cur_pe, cur_ev, np.mean([v for v in pe_vals if 0<v<150]) if pe_vals else 0, 22.0, cur_pb, min([v for v in pb_vals if 0<v<150]) if pb_vals else 0, wacc, roic, debt_to_ebitda, op_margins)

        dts = hist[hist.index >= target_dt].index
        def ret(days): return (hist['Close'].iloc[dts.searchsorted(dts[0]+pd.Timedelta(days=days))] - ep)/ep if dts.searchsorted(dts[0]+pd.Timedelta(days=days)) < len(dts) else None
        ...
        # V6.7: 即便失敗，也會回傳錯誤資訊在表格中，絕不讓表格變空白
        return {'代碼': sym, '名稱': '-', '進場日': target_date, '進場價': 0, '現價': 0, '當時總分': 0, '當時狀態': f"⚠️ 無法計算: {str(e)[:15]}", '當時合理價': 0, '當時PE': 0, '3個月': "-", '6個月': "-", '12個月': "-", '至今報酬': "-", 'Raw': 0}
...
cols_display = ['股票代碼', '名稱', '現價', '營收成長率', '預估EPS', '營業利益率', '淨利率', 'P/E (TTM)', 'P/B (Lag)', 'EV/EBITDA', '預估範圍P/E', '預估範圍P/B', '預估範圍P/S', '預估範圍EV/EBITDA', 'DCF/DDM合理價', '狀態', 'vs產業PE', '選股邏輯']
...
with tab1:
    ...
    if df_all.empty: st.error("❌ 找不到 tw_stock_list.csv。")
        ...
        if st.button("執行產業掃描", type="primary") and selected_inds:
            ...
            for idx, ind in enumerate(selected_inds):
                ...
                for sym in targets:
                    try:
                        ...
                        rng, avg_pe, min_pb = get_historical_metrics(stock, stock.history(period="10y"))
                        eps = info.get('trailingEps', 0); c_pe = p/eps if eps>0 else 0
                        if 0<c_pe<120: ind_pes.append(c_pe)
                        c_ev = info.get('enterpriseToEbitda', 0) or ((p*(info.get('sharesOutstanding',1) or 1))+info.get('totalDebt',0)-info.get('totalCash',0))/(info.get('ebitda',1) or 1)
                        is_fin = any(x in ind for x in ["金融", "保險"])
                        intrin, g, wacc, roic = get_3_stage_valuation(stock, is_fin, real_g)
                        raw_data.append((sym, ind, stock, info, p, real_g, qoq_g, wacc, roic, rng, avg_pe, min_pb, c_pe, c_ev, intrin, (intrin-p)/p if intrin>0 else -1, eps, is_fin))
                    except: pass
            ...
            status_text.text("✅ 完成！")
            if all_data: st.download_button("📥 下載完整名單", pd.DataFrame(all_data).sort_values('Total_Score', ascending=False)[cols_display].to_csv(index=False).encode('utf-8-sig'), f"V6.7_Scan.csv", "text/csv")
        ...
        if st.button("查詢", type="primary"):
            ...
            with st.spinner("查詢中..."):
                try:
                    ...
                    rng, avg_pe, min_pb = get_historical_metrics(stock, stock.history(period="10y"))
                    eps = info.get('trailingEps', 0); c_pe = p/eps if eps>0 else 0
                    c_ev = info.get('enterpriseToEbitda', 0)
                    is_fin = "Financial" in info.get('sector', '')
                    intrin, g, wacc, roic = get_3_stage_valuation(stock, is_fin, real_g)
                    data = compile_stock_data(sym, info.get('industry', ''), stock, info, p, real_g, qoq_g, wacc, roic, rng, avg_pe, min_pb, c_pe, c_ev, intrin, (intrin-p)/p if intrin>0 else -1, eps, is_fin, 22.0)
                ...
                except Exception as e: st.error("查無資料")
...
with tab3:
    ...
    if run_bt:
        res_bt, pb, t_list = [], st.progress(0), [t.strip() for t in t_input.split(',')]
        for i, sym in enumerate(t_list):
            res_bt.append(run_pit_backtest(sym, yf.Ticker(sym), s_date.strftime('%Y-%m-%d'), "Financial" in yf.Ticker(sym).info.get('sector', '')))
