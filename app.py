    ...
    if df_all.empty:
        ...
        if st.button("執行產業掃描", type="primary") and selected_inds:
            ...
            if all_scanned_data:
                ...
                csv = df_all_res.to_csv(index=False).encode('utf-8-sig')
                st.download_button(
                    label="📥 下載完整掃描名單 (CSV)",
                    data=csv,
                    file_name=f"V6.4_Scanned_Results_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv"
                )
