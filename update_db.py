...
# 1. 🔑 API 與更新參數設定
# ==========================================
API_TOKEN = os.getenv("FINMIND_TOKEN", "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkYXRlIjoiMjAyNi0wMy0yOCAyMDo0MjowNCIsInVzZXJfaWQiOiJlcmljY2hpIiwiZW1haWwiOiJlcmljLmNoaTE5ODhAZ21haWwuY29tIiwiaXAiOiI4Ny4yNDkuMTM4Ljc5In0.Kr93MomHC6qtFsyVhYBy5sgADiyW_zRM6i21UNkkpvw") 
api = DataLoader()
...
try:
    ...
    print("\n🎊 任務圓滿結束！您的 V8.0 系統現在已經充滿 2026 年的最新火力了！")
    ...
    # --- 自動同步 GitHub ---
    print("\n🚀 偵測到本地更新完成，正在自動同步至 GitHub...")
    try:
        # 1. Add 更新後的 Parquet
        subprocess.run(["git", "add", "tw_is_lite.parquet", "tw_bs_lite.parquet", "tw_cf_lite.parquet"], check=True)
        # 2. Commit
        msg = f"Auto-update quarterly reports: {datetime.now().strftime('%Y-%m-%d')}"
        subprocess.run(["git", "commit", "-m", msg], check=True)
        # 3. Push
        subprocess.run(["git", "push"], check=True)
        print("✅ GitHub 同步成功！")
    except Exception as e:
        print(f"❌ 自動同步失敗 (可能是無變更或 Git 未設定): {e}")
